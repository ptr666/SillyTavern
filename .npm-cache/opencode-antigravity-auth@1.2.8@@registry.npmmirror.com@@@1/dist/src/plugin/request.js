...
import { resolveModelWithTier, resolveModelWithVariant, isClaudeModel, isClaudeThinkingModel, CLAUDE_THINKING_MAX_OUTPUT_TOKENS, } from "./transform";
            ...
            if (isWrapped) {
                ...
                if (Array.isArray(requestPayload.tools)) {
                    if (isClaude) {
                        ...
                        requestPayload.tools = flattenedTools.map((tool, toolIndex) => {
                            const newTool = { ...tool };
                        ...
                        const normalizedTools = requestPayload.tools;
                        const geminiDeclarations = normalizedTools.map((tool) => ({
